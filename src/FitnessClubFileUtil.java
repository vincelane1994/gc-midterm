...
public class FitnessClubFileUtil {
private static FileLinesHelper linesHelper = new FileLinesHelper("clubs.txt");
	...
	private static Club convertLineToItemClub(String line) {
	...
	private static String convertItemToLineClub(Club club) {
		return String.format("%s\t%d", club.getName(), club.getAdress());
	}

	public static List<Club> readFile() {
		List<String> lines = linesHelper.readFile();
		List<Club> items = new ArrayList<>(lines.size());//pre=allocating enough memory by setting the max to the same as lines list
		for (String line : lines) {
			items.add(convertLineToItemClub(line));
		}
		return items;
//		Or with streams...
